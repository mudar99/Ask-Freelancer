...
import { GetProfileInfo, GetAmount, ShowProfileInfo, GetUserPosts, GetPrtojectsAPI, GetChat, local, GetOrders } from '../API';
...
class Profile extends Component {
    ...
    componentDidMount() {
        ...
        if (this.state.userID != null && (this.state.userID != this.state.myID && this.state.userID.length != 0)) {
            ...
            axios.get(GetChat + this.state.userID).then(
                res => {
                    if (res.data.status == true) {
                        console.log(res)
    ...
    render() {
        ...
        return (
            <main className="Profile lightMode">
                ...
                {this.state.userInfo.type === 0 && <Skills
                    Skills={this.state.skillsInfo}
                    userID={this.state.userInfo.id}
                    myID={this.state.myID} />}
                ...
                <Contact /> */}
                {this.state.check &&
                    <Chat userID={this.state.userInfo.id} RoomID={this.state.RoomID} isChatOn={this.state.isChatOn} />
                }
                <Footer />
            </main>
        );
